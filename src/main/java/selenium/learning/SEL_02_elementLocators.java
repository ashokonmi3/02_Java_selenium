...
//Reason: XPath provides powerful traversal of the HTML document, but it can be slower and less readable than CSS selectors.
//5. Class Name:
//Priority: Moderate to Low
//Reason: While classes can be shared among multiple elements, they're still useful, especially when elements have unique classes.
//6. Tag Name:
//Priority: Low
//Reason: Tag names are the least specific. Using them alone might result in locating multiple elements, unless combined with other attributes or strategies.
//7. Link Text & Partial Link Text:
//Priority: Specific to links
//Reason: These are specific to anchor (<a>) elements and are used when locating links by their visible text.
//Considerations:
//Uniqueness: Prioritize locators that ensure unique identification of elements to avoid ambiguity.
//Readability and Maintenance: Choose locators that are readable and maintainable for efficient script maintenance.
//Performance: Prefer faster locators (like ID and CSS selectors) for better performance.
//When creating automation scripts, it's a good practice to use a combination of locators, starting with the most reliable ones and employing fallback strategies if needed. Always test and verify locator reliability across different browsers and environments.
//============================
...
//	public static void main(String[] args) throws InterruptedException {
//		WebDriver driver = new ChromeDriver();
//		// // // BY link text - no need of plugin .its visible element. No need to inspect element
//		driver.get("https://www.google.co.in");
//		Thread.sleep(3000);
//
